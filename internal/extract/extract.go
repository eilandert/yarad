...
const Version = ""
...
func ExtractWithOptions(buf []byte, opts *Options) (res Result) {
	...
	// PE/ELF structural analysis: scan the original buffer and all streams
	// (including carved PE children) for entropy anomalies, virtual sections,
...
func fromOLE(buf []byte, res *Result, bud *archiveBudget, depth int, deadline time.Time) {
	...
	if err != nil {
		...
		fromWordFibEncryption(ole, res)   // Word 97-2003 FibBase fEncrypted bit
		fromPPTEncryptedSummary(ole, res) // legacy PPT EncryptedSummary storage
