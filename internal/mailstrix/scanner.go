...
type Scanner struct {
	...
	bigSrcDir          string        // MAILSTRIX_BIGFILE_RULES when it is a directory of sources
	bigSrcFile         string        // MAILSTRIX_BIGFILE_RULES when it is a precompiled .yac
...
func NewScanner(cfg *Config, logf func(string, ...any)) (*Scanner, error) {
	...
	if cfg.BigFileThreshold > 0 && cfg.BigFileRules != "" {
		if fi, err := os.Stat(cfg.BigFileRules); err == nil {
			...
			logf("WARNING: MAILSTRIX_BIGFILE_RULES=%s not found: %v (oversized-buffer gate will fall back to the full ruleset)", cfg.BigFileRules, err)
		}
	}
